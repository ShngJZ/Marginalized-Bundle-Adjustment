...
def eth3d_evaluate(
    ...
    poses_w2c_gt_i2j, poses_w2c_et_i2j = list(), list()
    for i in range(len(gt_pose_w2c)):
        for j in range(len(gt_pose_w2c)):
            if i != j:
                poses_w2c_gt_i2j.append(torch.inverse(gt_pose_w2c[j] @ torch.inverse(gt_pose_w2c[i])))
                poses_w2c_et_i2j.append(torch.inverse(pr_pose_w2c[j] @ torch.inverse(pr_pose_w2c[i])))

    poses_w2c_gt_i2j = torch.stack(poses_w2c_gt_i2j, dim=0)
    poses_w2c_et_i2j = torch.stack(poses_w2c_et_i2j, dim=0)
    err_rot = rotation_angle(
        rot_gt=poses_w2c_gt_i2j[:, 0:3, 0:3], rot_pred=poses_w2c_et_i2j[:, 0:3, 0:3]
    )
    err_tls = translation_angle(
        tvec_gt=poses_w2c_gt_i2j[:, 0:3, 3], tvec_pred=poses_w2c_et_i2j[:, 0:3, 3]
    )

    err_pose, _ = torch.max(torch.stack([err_rot, err_tls], dim=1), dim=1)
    aucs = pose_auc(err_pose.cpu().numpy(), thresholds=[1, 3, 5])
    result = {
        "err_rot": (err_rot < 5).float().mean().item(),
        "err_tls": (err_tls < 5).float().mean().item(),
        "auc-1": aucs[0],
        "auc-3": aucs[1],
        "auc-5": aucs[2],
    }

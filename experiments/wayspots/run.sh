...
DATA_ROOT="/fsx/users/shngjz/Marginalized-Bundle-Adjustment-Datasets/wayspots"
OUTPUT_LOC="/fsx/users/shngjz/Marginalized-Bundle-Adjustment/release/wayspots"

# Stage 1: Preprocessing (8 GPUs in parallel, wait for all to finish)
echo "=== Stage 1: Preprocessing ==="
for GPU_ID in $(seq 0 7); do
    CUDA_VISIBLE_DEVICES=${GPU_ID} python experiments/wayspots/preprocess.py \
        --data-root ${DATA_ROOT} \
